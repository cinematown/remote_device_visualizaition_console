...
void MetricsChartWidget::paintEvent(QPaintEvent*)
{
    ...
    const QRectF plot_area = bounds.adjusted(42, 16, -16, -30);
    painter.setPen(QPen(QColor(208, 213, 221), 1));
    painter.drawRect(plot_area);

    painter.setFont(QFont("Sans Serif", 9));
    painter.setPen(QColor(95, 99, 104));
    painter.drawText(bounds.adjusted(12, 4, -12, -4), Qt::AlignTop | Qt::AlignLeft, "Metrics");
    painter.drawText(
    ...
    painter.drawText(
        ...
        QString("Msg/s %1   Active %2").arg(latest.msg_per_sec).arg(latest.active));
}
